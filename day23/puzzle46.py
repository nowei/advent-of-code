...
with open(file, "r") as f:
    ...
    level4 = f.readline().strip().split('#')[1:]
level2 = ['D','C','B','A']
level3 = ['D','B','A','C']
...
def get_moves(state, i, cost, room, turn, history):
    ...
    if room:
        ...
        if (len(rooms[goal]) == 0 or all([roomie == cand for roomie in rooms[goal]])) and nothing_between:
            ...
            heuristic_cost = cost + moves * move_cost + turn_cost * turn
            cand_moves.append([heuristic_cost, cost + moves * move_cost, new_state, True, turn, history + [new_state]])
        else:
            ...
            for rest in valid_rests:
                ...
                new_cost = cost + moves * move_cost
                heuristic_cost = cost + moves * move_cost + turn_cost * turn
                cand_moves.append([heuristic_cost, new_cost, new_state, False, turn + 1, history + [new_state]])
    else:
        if (len(rooms[goal]) == 0 or all([roomie == cand for roomie in rooms[goal]])) and nothing_between:
            get_in_moves = room_length - len(rooms[goal])
            ...
            new_state = (new_hallway, new_rooms)
            new_cost = cost + moves * move_cost
            heuristic_cost = new_cost + turn_cost * turn
            cand_moves.append([heuristic_cost, new_cost, new_state, True, turn, history + [new_state]])
    return cand_moves
...
while moves:
    ...
    print('considering w/ heuristic {} cost {} turn {} {}                '.format(hc, c, turn, s), end='\r')
    if plopped:
