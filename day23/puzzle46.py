...
def validate_end(state):
    hallway, rooms = state
    conds = []
    conds.append(len(rooms[0]) == room_length and all([p == 'A' for p in rooms[0]]))
    conds.append(len(rooms[1]) == room_length and all([p == 'B' for p in rooms[1]]))
    conds.append(len(rooms[2]) == room_length and all([p == 'C' for p in rooms[2]]))
    conds.append(len(rooms[3]) == room_length and all([p == 'D' for p in rooms[3]]))
    return all(conds)

complete_state = ('...........', [['A', 'A', 'A', 'A'], ['B', 'B', 'B', 'B'], ['C', 'C', 'C', 'C'], ['D', 'D', 'D', 'D']])
