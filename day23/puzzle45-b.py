...
def get_valid_moves(state, history, heuristic_cost=0, cost=0, turn=0):
    hallway, rooms = state
    moves_to_consider = []

    for i, v in enumerate(hallway):
        if v != '.':
            moves_to_consider.extend(get_moves(state, i, cost, heuristic_cost, False, turn, history))
    if rooms[0] and not all([p == 'A' for p in rooms[0]]):
        moves_to_consider.extend(get_moves(state, 0, cost, heuristic_cost, True, turn, history))
    if rooms[1] and not all([p == 'B' for p in rooms[1]]):
        moves_to_consider.extend(get_moves(state, 1, cost, heuristic_cost, True, turn, history))
    if rooms[2] and not all([p == 'C' for p in rooms[2]]):
        moves_to_consider.extend(get_moves(state, 2, cost, heuristic_cost, True, turn, history))
    if rooms[3] and not all([p == 'D' for p in rooms[3]]):
        moves_to_consider.extend(get_moves(state, 3, cost, heuristic_cost, True, turn, history))
...
def get_moves(state, i, cost, heuristic_cost, room, turn, history):
    hallway, rooms = state
    cand_moves = []
    if room:
        cand = rooms[i][0]
    ...
    if room:
        ...
        if (len(rooms[goal]) == 0 or all([roomie == cand for roomie in rooms[goal]])) and nothing_between:
            ...
            new_heuristic_cost = cost + moves * move_cost
            cand_moves.append([new_heuristic_cost, cost + moves * move_cost, new_state, True, turn, history + [new_state]])
        else:
            ...
            for rest in valid_rests:
                ...
                new_cost = cost + moves * move_cost + turn_cost
                new_heuristic_cost = cost + moves * move_cost + turn_cost
...
while moves:
    ...
    if plopped:
        next_moves = get_valid_moves(s, history, hc, c, turn)
