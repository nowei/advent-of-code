...
def get_moves(state, i, cost, room, turn, history):
    ...
    if room:
        ...
        if (len(rooms[goal]) == 0 or all([roomie == cand for roomie in rooms[goal]])) and nothing_between:
            ...
            cand_moves.append([new_cost + heuristic_cost, new_cost, new_state, True, turn, history + [new_state]])
        else:
            ...
            for rest in valid_rests:
                ...
                new_cost = cost + mc
                heuristic_cost = estimate_cost_to_rest(new_state) 
                cand_moves.append([new_cost + heuristic_cost, new_cost, new_state, False, turn + 1, history + [new_state]])
    else:
        if (len(rooms[goal]) == 0 or all([roomie == cand for roomie in rooms[goal]])) and nothing_between:
            get_in_moves = room_length - len(rooms[goal])
            ...
            mc = moves * move_cost
            new_cost = cost + mc
            new_heuristic_cost = estimate_cost_to_rest(new_state) 
            cand_moves.append([new_cost + heuristic_cost, new_cost, new_state, True, turn, history + [new_state]])
    return cand_moves
