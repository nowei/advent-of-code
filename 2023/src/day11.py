...
class Galaxy11:
    ...
    def scaled_dist(
        ...
        return (x_o - x_i - x_empties + x_empties * galaxy_scaling) + (
            y_o - y_i - y_empties + y_empties * galaxy_scaling
        )
...
class Galaxies11:
    ...
    def __init__(self, orig_map):
        ...
        for i in range(len(orig_map[0])):
            if all(
                [val == "." for j in range(len(orig_map)) for val in orig_map[j][i]]
            ):
...
def solve_day11_part2(input: Galaxies11, scaling=1) -> int:
    ...
    for i in range(len(input.galaxies) - 1):
        for j in range(i + 1, len(input.galaxies)):
            ans += input.galaxies[i].scaled_dist(
                input.galaxies[j],
                scaling,
                input.empty_galaxy_col,
                input.empty_galaxy_row,
            )
    return ans
