    ...
    with open('output24.txt', 'w') as f:
        for args in operations:
            ...
            if op == 'inp':
                ...
                b = str(args[2]) if (args[2].isnumeric() or (args[2][0] == '-' and args[2][1:].isnumeric())) else args[2] + str(num[args[2]])
                if op == 'add':
                    result = '({}+{})'.format(a, b)
                elif op == 'mul':
                ...
                try:
                    ...
                    print(args[1]+str(num[args[1]] + 1), vals[args[1]+str(num[args[1]] + 1)])
                    mapping[args[1]] = str(vals[args[1]+str(num[args[1]] + 1)])
                except Exception as e:
                    print(e)
                    mapping[args[1]] = result
                line += ('{}='.format(args[1]+str(num[args[1]] + 1))+mapping[args[1]]).ljust(20)
            num[args[1]] += 1
            f.write(line + '\n')
# code_gen(operations)
# Some conditions
...
def process_op(args, inputs, mapping, num):
    ...
    if op == 'inp':
        ...
        a = mapping[args[1]]
        b = int(args[2]) if (args[2].isnumeric() or (args[2][0] == '-' and args[2][1:].isnumeric())) else mapping[args[2]] 
        label_a = args[1] + str(num[args[1]])
        label_b = str(args[2]) if (args[2].isnumeric() or (args[2][0] == '-' and args[2][1:].isnumeric())) else args[2] + str(num[args[2]])
        if op == 'add':
            result = a + b
            line += '{} = {} + {} = {} + {} = {}'.format((args[1] + str(num[args[1]] + 1)).ljust(4), label_a.rjust(5), label_b.ljust(6), str(a).rjust(7), str(b).ljust(6), result).ljust(12)
        elif op == 'mul':
            result = a * b
            line += '{} = {} * {} = {} * {} = {}'.format((args[1] + str(num[args[1]] + 1)).ljust(4), label_a.rjust(5), label_b.ljust(6), str(a).rjust(7), str(b).ljust(6), result).ljust(12)
        elif op == 'div':
            result = a // b
            line += '{} = {} // {} = {} // {} = {}'.format((args[1] + str(num[args[1]] + 1)).ljust(4), label_a.rjust(5), label_b.ljust(5), str(a).rjust(7), str(b).ljust(5), result).ljust(12)
        elif op == 'mod':
            result = a % b
            line += '{} = {} % {} = {} % {} = {}'.format((args[1] + str(num[args[1]] + 1)).ljust(4), label_a.rjust(5), label_b.ljust(6), str(a).rjust(7), str(b).ljust(6), result).ljust(12)
        elif op == 'eql':
