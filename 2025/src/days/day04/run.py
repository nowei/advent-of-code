...
def part2(_input: InputType) -> int:
    ...
    new_input = [list(s) for s in _input]
    while rolls_removed:
        rolls_removed = False
        changing_rolls = []
        for i in range(len(new_input)):
            for j in range(len(new_input[0])):
                if new_input[i][j] != "@":
                    continue
                num_rolls = 0
                for d_i in range(-1, 2):
                    for d_j in range(-1, 2):
                        if d_i == 0 and d_j == 0:
                            continue
                        if i + d_i < 0 or i + d_i >= len(new_input):
                            continue
                        if j + d_j < 0 or j + d_j >= len(new_input[0]):
                            continue
                        if new_input[i + d_i][j + d_j] == "@":
                            num_rolls += 1
                if num_rolls < 4:
