...
for j in range(1, len(area[0]) - 1):
    if area[0][j] < area[0][j - 1] and area[0][j] < area[0][j + 1] and area[0][j] < area[1][j]:
        risk_areas.append(area[0][j])
    if area[-1][j] < area[-1][j - 1] and area[-1][j] < area[-1][j + 1] and area[-1][j] < area[-2][j]:
        risk_areas.append(area[-1][j])
# row
for i in range(1, len(area) - 1):
    if area[i][0] < area[i - 1][0] and area[i][0] < area[i + 1][0] and area[i][0] < area[i][1]:
        risk_areas.append(area[i][0])
    if area[i][-1] < area[i - 1][-1] and area[i][-1] < area[i + 1][-1] and area[i][-1] < area[i][-2]:
