...
for i in range(len(numbers)):
    ...
    unknowns = {
        5: [v for v in nums if len(v) == 5],
        6: [v for v in nums if len(v) == 6],
    }
    # 3 if right side matches 1
    mapping[3] = next(
        cand for cand in unknowns[5] if all(m in cand for m in mapping[1])
    )
    # 6 if not all right side matches 1
    mapping[6] = next(
        cand for cand in unknowns[6] if not all(m in cand for m in mapping[1])
    )
    # 2 if 4 lines matches 6 and not 3
    mapping[2] = next(
        cand
        for cand in unknowns[5]
        if cand != mapping[3] and sum([1 if m in cand else 0 for m in mapping[6]]) == 4
    )
    # 5 is remaining of [2, 3, 5]
    mapping[5] = next(
        cand for cand in unknowns[5] if cand not in [mapping[2], mapping[3]]
    )
    # 0 if 4 lines matches 5
    mapping[0] = next(
        cand for cand in unknowns[6] if sum([1 for m in cand if m in mapping[5]]) == 4
    )
    # 9 if not 0 and not 6
    mapping[9] = next(
        cand for cand in unknowns[6] if cand not in [mapping[0], mapping[6]]
    )
    for k in mapping:
        ans_map[mapping[k]] = k
